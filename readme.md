...
All database migrations and schema management are handled via **Alembic** and the helper script `setup.py`. This script provides commands for revising, migrating, resetting, and managing DB tables and MinIO bucket.
...
python -m app.setup bucket create                  # Create MinIO bucket
python -m app.setup bucket delete                  # Delete MinIO bucket
```
