...
def run_endpoint_test(base_url: str):
    ...
    resp = requests.post(token_url, data=VALID_EXECUTIVE_CREDENTIALS["admin"])
    ...
    resp = requests.get(token_url, headers=ADMIN_HEADER)
    ...
    resp = requests.post(token_url, data=VALID_EXECUTIVE_CREDENTIALS["guest"])
    ...
    resp = requests.delete(f"{token_url}/{GUEST_ID}", headers=GUEST_HEADER)
    ...
    GUEST_EX_ID = resp.json()["executive_id"]
    resp = requests.get(token_url, headers=GUEST_HEADER, params={"id": GUEST_ID})
    assert resp.json() == []
    assert resp.status_code == 200
    ...
    tokens = resp.json()
    ID = tokens[-1]["id"]
    resp = requests.delete(f"{token_url}/{ID}", headers=GUEST_HEADER)
    assert resp.status_code == 204
    resp = requests.get(token_url, headers=GUEST_HEADER, params={"id": ID})
    assert resp.json() == []
    assert resp.status_code == 200
    ...
    resp = requests.post(f"{token_url}/revoke")
    assert resp.status_code == 401
    ...
    resp = requests.get(token_url, headers=ADMIN_HEADER, params={"id": 0})
    assert resp.json() == []
    assert resp.status_code == 200

    # -------------------------------------------------------------
    print("CASE 23: Guest attempts to revoke admin token using ID → 403")
    resp = requests.delete(f"{token_url}/{ADMIN_ID}", headers=GUEST_HEADER)
    ...
    assert resp.status_code == 200
    resp = requests.get(token_url, headers=ADMIN_HEADER, params={"id": ADMIN_ID})
    assert resp.json() != []
    assert resp.status_code == 200

    # -------------------------------------------------------------
    print("CASE 25: Admin deletes token using id=0 → 204")
    resp = requests.delete(f"{token_url}/0", headers=ADMIN_HEADER)
    assert resp.status_code == 204
    ...
    assert resp.status_code == 200
    resp = requests.get(token_url, headers=ADMIN_HEADER, params={"id": GUEST_ID_2})
    assert resp.json() != []
    assert resp.status_code == 200
    ...
    for token in tokens:
        ID = token["id"]
        resp = requests.delete(f"{token_url}/{ID}", headers=ADMIN_HEADER)
        assert resp.status_code == 204

    # -------------------------------------------------------------
    print("CASE 29: Admin deletes guest token using its ID → 204")
    resp = requests.delete(f"{token_url}/{ADMIN_ID}", headers=ADMIN_HEADER)
    assert resp.status_code == 204
