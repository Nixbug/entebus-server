...
class UpdateForm(BaseModel):
	...
	ticket_mode: TicketingMode = Field(default=None, description=enum_str(TicketingMode))
...
def search_service_automations(
	...
	if query_params.starting_at_ge is not None:
		query = query.filter(ServiceAutomation.starting_at >= query_params.starting_at_ge)
	if query_params.starting_at_le is not None:
		query = query.filter(ServiceAutomation.starting_at <= query_params.starting_at_le)
