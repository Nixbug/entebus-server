...
async def create_token(
    ...
    **Issue a new access token for an executive after validating credentials.**
    ...
    - **Token Creation**
        ...
        - The access token's expiration duration is stored in `expires_in` (as an integer value representing ACCESS_TOKEN_VALIDITY in seconds).
        - The refresh token's expiration timestamp is stored in `expires_at` (as a datetime value representing REFRESH_TOKEN_VALIDITY in seconds).
